...
At the moment the code creates a summary table. It does not return a table of replications versus performance measures. This is necessary for charts. Function create_summary_table needs to be broken up. A function that first assembles the replication results would be better.

column 1: arrivals_by_replication
...
resource_waiting_times_by_replication <- function(reps) {
  ...
  # long to wide format...
  waiting_times_wide <- spread(waiting_times_long, resource, rep_waiting_time)
  return(waiting_times_wide)
}

resource_waiting_times_by_replication(reps)
```
...
resource_utilisation_by_replication <- function(reps, results_collection_period){
  ...
  # add resource counts to table for calc
  util_long <- merge(util_long, get_resource_counts(exp), 
                     by="resource", all=TRUE)
    
  # total in use / total scheduled time
  util_long$util = util_long$in_use / (results_collection_period * util_long$count)  
  
  # drop activity time and count and recode names
  util_long <- subset(util_long, select = c(replication, resource, util)) %>% 
    mutate(resource=recode(resource,
                       'triage_bay'='01b_triage_util',
                       'registration_clerk'='02b_registration_util',
                       'examination_room'='03b_examination_util',
  ...
  # long to wide format...
  util_wide <- spread(util_long, resource, util)
    
  return(util_wide)
}

resource_utilisation_by_replication(reps, DEFAULT_RESULTS_COLLECTION_PERIOD)
```
