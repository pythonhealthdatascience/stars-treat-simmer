...
system_kpi_by_replication(reps)
```
...
histogram_of_replications <- function(rep_table, column_name, unit_label, n_bins=30){
  
  # Divide the x range for selected column into n_bins
  binwidth <- diff(range(select(rep_table, all_of(column_name))))/n_bins

  # top/tail with "`" as numeric in string.
  g <- ggplot(rep_table, aes_string(paste("`", column_name, "`", sep=""))) +
...
#histogram_of_replications("`01a_triage_wait`", "mins")
...
print(create_summary_table(rep_table, exp))
