...
create_non_trauma_pathway <- function(exp){
  ...
  non_trauma_pathway <- trajectory(name="non_trauma_pathway") %>%
    ...
    timeout(task = function() rlnorm(1, DEFAULT_REG_PARAMS$mu, DEFAULT_REG_PARAMS$sigma)) %>%
...
```{r}
default_experiment <- create_experiment(log_level=1)
treat_sim <- get_model(default_experiment)

treat_sim %>% 
  run(until=DEFAULT_RESULTS_COLLECTION_PERIOD) %>% 
  invisible
```
