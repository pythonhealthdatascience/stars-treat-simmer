...
> Note: we are calculating KPIs uses our own code here, but you can also use `simmer.plot`. Help with install of `simmer.plot` (`igraph` installation is the actual issue) on Linux based systems: <https://r.igraph.org/articles/installation-troubleshooting#cannot-compile-igraph-from-sources-on-linux>
>
...
```{r}
get_throughput <- function(data_wide){
  return(sum(data_wide$departed, na.rm=TRUE))
}
...
calc_kpi_from_attributes_for_rep <- function(rep){
  ...
  throughput <- get_throughput(data_wide)
  
  # store results in list
  # rep_results = list("total_time_nt" = mean_time_1, "total_time_t" = mean_time_2, 
  #                    "throughput"=throughput)
  
  rep_results <- data.frame("05_total_time(non-trauma)" = mean_time_1,
                            "08_total_time(trauma)" = mean_time_2, 
