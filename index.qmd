...
TRAUMA_TREATMENT_PARAMS <- normal_moments_from_lognormal(30.0, sqrt(4.0))

# Non trauma treatment (lognormal distribution)
TRAUMA_TREATMENT_PARAMS <- normal_moments_from_lognormal(13.3, sqrt(2.0))
...
trauma_pathway <- trajectory(name="trauma_pathway") %>%
  ...
  set_attribute("start_trauma_room_wait", function() {now(env)}) %>%
  seize(resource = "trauma_room", amount = 1) %>%
  timeout(task = function() rexp(1, 90.0)) %>%
  release(resource = "trauma_room", amount = 1) %>%
  log_(function() {paste("(T) Trauma room wait time:",
                   now(env) - get_attribute(env, "start_trauma_room_wait"))}) %>%
  
  # request treatment cubicle
  seize(resource = "trauma_treat_cubicle", amount = 1) %>%
  timeout(task = function() rlnorm(1, TRAUMA_TREATMENT_PARAMS$mu,
                                   TRAUMA_TREATMENT_PARAMS$sigma)) %>%
...
non_trauma_pathway <- trajectory(name="non_trauma_pathway") %>%
  ...
  timeout(task = function() rlnorm(1, DEFAULT_REG_PARAMS$mu,
                                   DEFAULT_REG_PARAMS$sigma)) %>%
  ...
  timeout(task = function() rnorm(1, 16.0, sqrt(4.0))) %>%
...
env <- simmer("TreatSim") %>% 
  ...
  add_generator("patient", patient_arrival, function() rexp(1, 1.0/10.0))
