...
```{r}
nspp_thinning <- function(simulation_time, data){
  
  # calc time interval: assumes intervals are of equal length
  interval <- data$period[2] - data$period[2]
...
patient <- trajectory("patient pathway") %>% 
  ...
  log_(function() {paste("Exit treatment pathway")})
  
```

Run the model with thinning

```{r}
env <- simmer("TreatSim") %>% 
  add_generator("patient", patient, 
                function() nspp_thinning(now(env), arrivals)) %>% 
  run(540.0)
```

```{r}
nspp_thinning(15, arrivals)
```
